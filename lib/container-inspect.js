...
function ContainerInspect(docker, cid) {
	...
	self.logger.debug(self.uuid12, "new container object with uuid12:", self.uuid12);
}
...
ContainerInspect.prototype.gatherInfo = function(cb) {
	...
	self.cont.inspect(function(err, data) {
		if (err) {
		...
		var iHPb = self.inspectData.NetworkSettings.Ports;
...
ContainerInspect.prototype.getImgName = function() {
	this.imgname = this.inspectData.Name.replace(/\.|\/|:/g, '');
};
...
ContainerInspect.prototype.getIps = function() {
	var self = this;
	var ips = {
		internal : self.inspectData.NetworkSettings.IPAddress,
		exposed : null
	};

	if (ips.internal == '') {
		Object.keys(self.inspectData.NetworkSettings.Networks).forEach(function(key) {
			if (self.inspectData.NetworkSettings.Networks[key].IPAddress != '') {
				ips.internal = self.inspectData.NetworkSettings.Networks[key].IPAddress;
			}
		});
	}
	
	try {
		var inspectedHostPorts = self.inspectData.NetworkSettings.Ports;
		self.logger.debug("inspectedHostPorts", inspectedHostPorts);

		if (inspectedHostPorts
				&& Object.keys(inspectedHostPorts).length > 0
				&& inspectedHostPorts[Object.keys(inspectedHostPorts)[0]]
				&& inspectedHostPorts[Object.keys(inspectedHostPorts)[0]]
				&& inspectedHostPorts[Object.keys(inspectedHostPorts)[0]][0]
				&& inspectedHostPorts[Object.keys(inspectedHostPorts)[0]][0].HostIp
		) {
			var hostBoundIp = inspectedHostPorts[Object.keys(inspectedHostPorts)[0]][0].HostIp;
