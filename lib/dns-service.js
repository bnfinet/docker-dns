...
server.on('query', function(query) {
	...
	case 'A':
		...
		break;
	case 'AAAA':
		if (_recs.aaaa[domain]) {
			query.addAnswer(domain, _recs.aaaa[domain]);
		} else if (_recs.cname[domain]) {
			query.addAnswer(domain, _recs.cname[domain]);
			logger.debug('cname aaaa: ', _recs.cname[domain].target,
				_recs.aaaa[_recs.cname[domain].target]);
			query.addAnswer(_recs.cname[domain].target,
				_recs.aaaa[_recs.cname[domain].target]);
		}
		server.send(query);
	...
	case 'CNAME':
		query.addAnswer(domain, _recs.cname[domain]);
		logger.debug('cname a + aaaa: ', _recs.cname[domain].target,
					_recs.a[_recs.cname[domain].target]);
		query.addAnswer(_recs.cname[domain].target,
				_recs.a[_recs.cname[domain].target]);
		query.addAnswer(_recs.cname[domain].target,
...
//		console.log("a records", _recs.a);
	...
	function(done) {
		_buildAAAArecords(recs, done);
//		console.log("aaaa records", _recs.aaaa);
	},
	function(done) {
...
var _buildAAAArecords = function(recs, cb) {
	_recs.aaaa = {};
	async.each(recs.a, function(a, done) {
		_recs.aaaa[a.fqdn] = new named.AAAARecord('::ffff:' + a.ip);
		done();
	}, cb);
