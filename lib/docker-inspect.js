...
function Dinspect(config) {
	...
	if (self.configure(config)) {
		...
		if (self.debug) {
			self.log(self.name, "new Dinspect for docker "
...
proto.inspectContainers = function(cb) {
	var self = this;
	if (self.debug) {
		self.log(self.name, "getting containers for docker "
				+ self.config.publicname);
	}
	self.containers = [];
	self.D.listContainers({
		all : 1
	}, function(err, containers) {
		if (self.debug) {
			self.log(self.name, "inspecting " + containers.length
					+ " containers");
		}
